...
export default function App() {
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent | TouchEvent) => {
      ...
      ctx.lineWidth = Math.min(22, 3 + speed * 0.4)
      ...
      ctx.lineWidth = Math.min(40, 8 + speed * 0.8)
      ctx.stroke()
      ...
      if (!isMuted && speed > 8 && arpRef.current && strokeCount.current % 6 === 0) {
        ...
        arpRef.current.triggerAttackRelease(note, '16n', undefined, 0.4)
      }
  ...
  return (
    <>
      {/* MUTE */}
      {screen === 'canvas' && (
        <button className={`mute-btn ${isMuted ? 'muted' : ''}`} onClick={handleMute} aria-label="mute">♪</button>
      )}
      ...
      {screen === 'weight' && (
        <div className="screen">
          ...
          <div className="weight-display" style={{
            color: `hsl(${200 + weight * 5}, 70%, 68%)`,
            textShadow: `0 0 40px hsla(${200 + weight * 5}, 70%, 68%, 0.5)`
          }}>{weight}</div>
        ...
        <div className="screen">
          ...
          <div className="feel-grid">
            {([
              ...
              <div key={f.key}
                className={`feel-card ${feel === f.key ? 'selected' : ''}`}
